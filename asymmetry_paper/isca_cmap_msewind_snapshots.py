...
def plot_gill_dev_isca(run, ax, pentad, land_mask=None):
    ...
    b = ax.quiver(data.lon[::6], data.lat[::3], data_zanom.ucomp.sel(pfull=850.,xofyear=pentad)[::3,::6], data_zanom.vcomp.sel(pfull=850.,xofyear=pentad)[::3,::6], scale=100, angles='xy', width=0.005, headwidth=3., headlength=5., zorder=3)
    ...
    ax.set_ylim(-60.,60.)
    ax.set_yticks(np.arange(-60.,60.,30.))
...
def plot_gill_dev_cmap(ax, pentad):
    ...
    mse = (mc.cp_air * data_t + mc.L * data_q + 9.81 * data_h)/1000.
    title = 'Pentad ' + str(int(pentad))      
    f1 = mse.sel(pentad=pentad,lev=85000.).plot.contourf(x='lon', y='lat', ax=ax, add_labels=False, add_colorbar=False, extend='both',  cmap='Blues', zorder=1, levels = np.arange(310.,351.,2.5))
    b = ax.quiver(data_u.lon[::6], data_u.lat[::3], data_u.sel(pentad=pentad)[::3,::6], data_v.sel(pentad=pentad)[::3,::6], scale=100, angles='xy', width=0.005, headwidth=3., headlength=5., zorder=3)
...
# Save as a pdf
plt.savefig(plot_dir + 'msewind_snapshots.pdf', format='pdf')
plt.close()
