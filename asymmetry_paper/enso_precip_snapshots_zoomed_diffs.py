...
def plot_gill_dev_cmap(ax, pentad, nino34='all'):
    ...
    data_slp.coords['enso_phase'] = (('pentad'), np.repeat(nino_34_march.enso_phase.values,73)) 
    ...
    data_precip.coords['enso_phase'] = (('time'), np.repeat(nino_34_march.enso_phase.values,73)) 
    ...
    data_u = pentad_mean(data_u, np.arange(1979,2017))
    data_u = data_u - data_u.mean('lon')
    data_u.coords['enso_phase'] = (('pentad'), np.repeat(nino_34_march.enso_phase.values,73)) 
    data_v = pentad_mean(data_v, np.arange(1979,2017))
    data_v = data_v - data_v.mean('lon')
    data_v.coords['enso_phase'] = (('pentad'), np.repeat(nino_34_march.enso_phase.values,73)) 
    
    if nino34=='all':
        data_precip = data_precip.groupby('pentad').mean('time')
        data_slp = data_slp.drop('enso_phase').groupby('pentad').mean('pentad')
...
# Start figure with 20 subplots
fig, ((ax1, ax2, ax3, ax4), (ax5, ax6, ax7, ax8), (ax9, ax10, ax11, ax12), (ax13, ax14, ax15, ax16), (ax17, ax18, ax19, ax20)) = plt.subplots(5, 4, sharey='row', sharex='col')
...
for ax in axes[::4]:
    f1 = plot_gill_dev_cmap(ax, pentads[i], nino34='all')
    i=i+1

i=0
for ax in axes[1::4]:
    f1 = plot_gill_dev_cmap(ax, pentads[i], nino34='ln')
    i=i+1

i=0 
for ax in axes[2::4]:
    f1 = plot_gill_dev_cmap(ax, pentads[i], nino34='ne')
    i=i+1
    
i=0
for ax in axes[3::4]:
    f1 = plot_gill_dev_cmap(ax, pentads[i], nino34='en')
    i=i+1
