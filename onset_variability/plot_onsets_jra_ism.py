...
onsets_ism = xr.DataArray(onsets_ism, coords={'year': ('year', range(1958,2017))}, dims=['year'])

years = range(1958,2017)

# Take mean and standard deviation
ism_stats = np.mean(np.array(onsets_ism.sel(year=years))), np.std(np.array(onsets_ism.sel(year=years)))
...
def get_timing(onsets, years):
    ...
    return onsets_out
#print(onsets_ism.swap_dims({'year': 'timing'}).sel(timing='Normal').year)

onsets_ism = get_timing(onsets_ism, years)
#print(onsets_ism.timing.values)

# Plot onsets 
plt.plot(years,onsets_ism,'o-', color='C0')
plt.plot([1958,2016],[round(ism_stats[0])]*2,'-', color='C0')
plt.fill_between([1958,2016], [round(ism_stats[0]) - 0.75*ism_stats[1], round(ism_stats[0]) - 0.75*ism_stats[1]], 
                              [round(ism_stats[0]) + 0.75*ism_stats[1], round(ism_stats[0]) + 0.75*ism_stats[1]], alpha=0.2, color='C0')
plt.xlabel('Year')
...
def plot_winter_climate(data, title, levels_clim=np.arange(-50.,51.,5.), levels=np.arange(-5.,5.1,0.5), local=False, lev=20000.):
    ...
    data.coords['timing'] = (('time'), np.repeat(onsets_ism.timing.values,12)) 
    ...
    if local:
        plt.savefig('/scratch/rg419/plots/onset_variability_new/early_vs_late/ism/JFM_' + title + '_local.pdf', format='pdf')
    else:
        plt.savefig('/scratch/rg419/plots/onset_variability_new/early_vs_late/ism/JFM_' + title + '.pdf', format='pdf')
    plt.close()
