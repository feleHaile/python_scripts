...
def set_plot_features(ax, title='', do_legend=True, legend_labels=[], fontsize=9, leg_title=None):
    """
    Inputs:
        ax - axis to modify
    ...
    if do_legend:
        legend = ax.legend(legend_labels, loc='upper left', borderaxespad=0., fontsize=fontsize, title=leg_title, ncol=3) #bbox_to_anchor=(1.05, 1),
...
if __name__ == "__main__":
    ...
    for run in runs:
        if run == 'sn_1.000':
            ...
            i=i+1
    set_plot_features(ax3, title='', do_legend=False) #, legend_labels=['0.5', '0.75', '1.0', '1.25', '1.5', '1.75', '2.'], leg_title='$\Omega$/$\Omega_{E}$')
    ...
    for run in runs_15:
        if run == 'mld_15':
            p_cent_grad_scatter(run, color='k', ax=ax5, linewidth=2.)    
        else:
            p_cent_grad_scatter(run, color=colors[i], ax=ax5, linewidth=1.)
            i=i+1
    set_plot_features(ax5, title='', legend_labels=['0.5', '0.75', '1.0', '1.25', '1.5', '1.75', '2.'], leg_title='$\Omega$/$\Omega_{E}$')
    ax5.set_ylabel('ITCZ migration rate')
    ax5.set_xlabel('ITCZ latitude')
    ...
    if errorbars:
        ...
        ax2.plot(rots, max_rate_rot_5.values, 'xr', mew=2, ms=10)
        ax2.plot(rots, max_rate_rot_15.values, 'xb', mew=2, ms=10)
        ax4.plot(rots, max_rate_lat_rot.values, 'xk', mew=2, ms=10, label='10m')
        ax4.plot(rots, max_rate_lat_rot_5.values, 'xr', mew=2, ms=10, label='5m')
    ...
    ax2.set_yticks(np.arange(0.1,0.8,0.1))
    ax4.set_ylabel('Latitude')
    ax4.set_xlabel('$\Omega$/$\Omega_{E}$')
    ax4.set_yticks(np.arange(2,17,2))
    ax4.set_ylim([2,16])
    ...
    legend = ax4.legend( loc='upper right', borderaxespad=0., fontsize=10, title='Mixed layer depth', ncol=2)
    ...
    # Save as a pdf
    plt.savefig(plot_dir + 'pcent_rate_rot_transpose.pdf', format='pdf')
    plt.close()
